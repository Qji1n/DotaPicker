...
namespace DotaProject
{
    ...
    public class OpenDotaApiService
    {
        ...
        private long maxMatchId = 9999999999999999;	
		...
		var response = await SharedHttpClient.GetAsync($"https://api.opendota.com/api/proMatches?" +
                    $"less_than_match_id={maxMatchId}");//($"{BaseUrlOpenDotaAPI}publicMatches?min_rank={minRank}");
        ...
        {
            var datasetDirectory = "dataset/proMatches";
        ...
        {
            var datasetDirectory = "dataset/proIDs";
            if (!Directory.Exists(datasetDirectory))
                Directory.CreateDirectory(datasetDirectory);
