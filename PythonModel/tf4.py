...
DIM = 4   # Количество контекстных героев
EMBEDDING_SIZE = 150
...
def get_context_and_label(team):
    combs = list(combinations(team, 4))
    for i in range(len(combs)):
        cbow_contexts.append(np.array(combs[i]))
...
def create_model():
    ...
    dense_out_1 = Dense(5, activation='softmax')(permute_1) #Было 5
    dense_out_2 = Dense(5, activation='softmax')(permute_2) #Было 5
    ...
    multiply_0_1 = Multiply()([lstm_out_1,permute_3])
    multiply_0_2 = Multiply()([lstm_out_2,permute_4])
...
model.fit(train_data, train_labels, validation_data=(val_data, val_labels),
          epochs=100, batch_size=2000, shuffle=False, callbacks=callbacks)
