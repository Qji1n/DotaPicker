...
def custom_metric(X, Y):
    NUM_IN_QUERY = 10000.0


    # Расширение Y для выполнения попарных операций
    Y_expanded = tf.expand_dims(Y, 1)

    # Вычисление совпадений между Y и Y
    matches_Y = tf.logical_and(Y[:, tf.newaxis, :], Y_expanded)
    NUM_IN_QUERY = tf.reduce_sum(tf.cast(matches_Y, tf.float32), axis=2)

    # Вычисление совпадений между X и Y
    matches = tf.logical_and(X[:, tf.newaxis, :], Y_expanded)
    and_sum = tf.reduce_sum(tf.cast(matches, tf.float32), axis=2)

    # Вычисление веса
    weight = tf.pow(and_sum / NUM_IN_QUERY, 4)

    # Вычисление расстояния
    distance = (1 / (weight + 1e-8)) - 1
...
class KNeighborsClassifier(object):
    ...
    def _predict(self, X):
        # Вычисление расстояний между обучающим набором и X
        distance = DISTANCE_FUNCTIONS[self.metric](self._fit_X, X)

        # Получение индексов и значений k ближайших соседей
        # Негативное значение расстояния для получения в порядке возрастания
        top_k_values, top_k_indices = tf.nn.top_k(-distance,
                                                  k=self.n_neighbors)
        ...
        if self._predict_proba:
            ...
            return top_k_values, top_k_indices, proba
        return top_k_values, top_k_indices, prediction
